...
function Todocomponent() {
  ...
  const filterIncompletedTodos = (todos) => todos && !todos.completed;
  ...
  return (
    <div>
      <div className="todoWrapper container-fluid">
        ...
        <ul>
          {todos.map((todo) => (
            <li key={todo.id}>
              ...
              <button
                className={todo.completed ? "bgBlue" : "bgRed"}
        ...
        <p className="footer">
          You have <span>{incompletedTaskCounts}</span> pending task
          {incompletedTaskCounts !== 1 ? "s" : ""}!
        </p>
      </div>
    </div>
  );
